defmodule MishkaInstaller.Installer.DepChangesProtector do
  ...
  def deps(app, first_binding) do
    GenServer.cast(__MODULE__, {:deps, app, first_binding})
  end
  ...
  @impl true
  def handle_info({:do_compile, app, first_binding}, state) do
    new_state =
      if is_nil(state.ref) do
        task =
        ...
        {:noreply, Map.merge(state, %{ref: task.ref, app: app, app_status: first_binding})}
      else
  ...
  @impl true
  def handle_cast({:deps, app, first_binding}, state) do
    Process.send_after(self(), {:do_compile, app, first_binding}, 100)
    {:noreply, state}
  end
