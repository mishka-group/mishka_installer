defmodule MishkaInstaller.Installer.Live.DepGetter do
  ...
  @impl Phoenix.LiveView
  def handle_event("save", %{"select_form" => "hex"} = _params, socket) do
    {:noreply, socket}
  end

  @impl Phoenix.LiveView
  def handle_event("for_select", %{"type" => type} = _params, socket) when type in ["upload", "hex", "git"] do
    socket =
      socket
  ...
  defp dep_form(:upload, assigns) do
    ...
    <section id="dep-getter" class="col-md-6 mx-auto" phx-drop-target={@uploads.dep.ref}>
      <form id="extensions-upload-form" phx-submit="save" phx-change="validate">
        ...
        <h2 class="mt-3 mb-1">Drop your file here or Click</h2>
        <%= live_file_input @uploads.dep, class: "d-none" %>
        <%= for entry <- @uploads.dep.entries do %>
          <br>
          <span phx-click="cancel-upload" phx-value-type-ref={entry.ref} aria-label="cancel">
          ...
          <span phx-click="cancel-upload" phx-value-type-ref={entry.ref} aria-label="cancel">&times;</span>
          <br>
        <% end %>
        ...
        <span class="mt-4 mb-4">
          <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="hex">Get from Hex</a> - <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="git">Or from Git</a>
        </span>
      </form>
    </section>
    """
  ...
  defp dep_form(:hex, assigns) do
    ~H"""
      ...
      <form id="extensions-upload-form" phx-submit="save" phx-change="validate">
        ...
        <br>
        <br>
        <input name="app" class="form-control form-control-lg mb-3" type="text" placeholder="Input app name like: mishka_installer">
        ...
        <br>
        <span class="mt-4 mb-4">
          <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="upload">Upload</a> - <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="git">Or get from Git</a>
        </span>
      </form>
      </section>
    """
  ...
  defp dep_form(:git, assigns) do
    ~H"""
      ...
      <form id="extensions-upload-form" phx-submit="save" phx-change="validate">
        ...
        <br>
        <br>
        <input name="url" class="form-control form-control-lg mb-3" type="text" placeholder="Your Git url">
        <br>
        ...
        <br>
        <span class="mt-4 mb-4">
          <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="upload">Upload</a> - <a class="dep-link text-decoration-none" phx-click="for_select" phx-value-type="hex">Or get from hex</a>
        </span>
      </form>
      </section>
    """
