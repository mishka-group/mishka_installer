defmodule MishkaInstaller.Installer.Live.DepGetter do
  ...
  defp dep_form(:installed_plugins, assigns) do
    ...
    <section id="dep-installed-plugins" class="col mx-auto">
      ...
      <div id="all_loaded_app" class="container">
        <%= for {app, des, ver} <- @loaded_apps do %>
          <%= if app in Enum.map(DepHandler.compare_dependencies_with_json(), & &1.app) do %>
              <span id={"#{app}" <> "dropdown-class"} class="dropdown" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title={des}>
          ...
          <% else %>
            <small id={app} class="d-inline-flex mx-1 mb-2 px-2 py-1 fw-semibold text-success bg-success bg-opacity-10 border border-success border-opacity-10 rounded-2" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title={des}>
              <%= "#{app}" %> in v<%= "#{ver}" %>
            </small>
          <% end %>
        <% end %>
  ...
  defp icon(assigns) do
    ...
    <p id="dep-icon-getter" class="col mx-auto text-end">
      ...
      <span id="plugin-icon" phx-click="form_select" phx-value-type="installed_plugins" phx-hook="EnableToolTip">
      ...
      <span phx-click="form_select" phx-value-type="upload">
