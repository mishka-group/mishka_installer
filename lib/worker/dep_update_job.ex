defmodule MishkaInstaller.DepUpdateJob do
  ...
  defp send_update_request_based_on_type({:ok, :read_dep_json, dependencies}), do: Enum.map(dependencies, &create_update_request/1)
  ...
  defp create_update_request(%{"type" => "git", "git_tag" => tag} = json_data) when tag in ["master", "main"] do
  ...
  defp create_update_request(%{"type" => "hex"} = json_data) do
    case Sender.package("hex", %{"app" => json_data["app"]}) do
      {:ok, :package, %{"latest_stable_version" => version}} ->
        {:ok, {String.to_atom(json_data["app"]), :hex, json_data["url"], version}, version_compare(version, json_data["version"], :hex, json_data["app"])}
      {:error, :package, error_status} ->
  ...
  defp version_compare(request_ver, json_ver, type, app) do
    ...
    _ ->
      MishkaInstaller.update_activity(%{app: app, type: type, status: :server_bad_version}, "high")
      false
  end
